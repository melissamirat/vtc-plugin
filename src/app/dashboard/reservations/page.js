// src/app/dashboard/reservations/page.js
"use client";
...
import {
  ...
  updateBookingStatus,
  getUserWidgets,
  deleteBooking, // <--- Import ajouté
} from "@/lib/firestore";

// NOUVEAU POPUP DE CONFIRMATION DE SUPPRESSION
function DeleteConfirmationModal({ isOpen, onClose, onConfirm, booking, isLoading }) {
      ...
      <div className="bg-white rounded-3xl shadow-2xl p-6 max-w-md w-full">
        ...
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
          <p className="text-sm text-yellow-800">
            ...
            La réservation sera définitivement retirée de votre liste.
          </p>
        </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm p-4">
      <div className="bg-white rounded-3xl shadow-2xl p-6 max-w-md w-full">
        ...
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
          <p className="text-sm text-yellow-800">
            <strong>⚠️ Attention :</strong> Un email de confirmation sera envoyé
            au client. Veuillez vous assurer de pouvoir assurer cette course
            avant de confirmer.
          </p>
        </div>
      ...
      <div className="bg-white rounded-3xl shadow-2xl p-6 max-w-md w-full">
        ...
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 mb-6">
          <p className="text-xs text-yellow-800">
            <strong>⚠️</strong> Un email d'annulation sera envoyé au client avec
...
export default function ReservationsPage() {
  ...
  // Pour le popup de suppression <--- NOUVEAU
  ...
  useEffect(() => {
    ...
    if (
      urlFilter &&
      ["all", "pending", "confirmed", "canceled"].includes(urlFilter)
    ) {
      setFilter(urlFilter);
    }
  }, [searchParams]);
  ...
  // Ouvrir le popup de suppression <--- NOUVEAU
  const handleDeleteClick = (booking) => {
    setSelectedBooking(booking);
    setShowDeleteModal(true);
  };

  // Annuler la réservation avec motif, envoyer email et update
  const handleCancelBooking = async (reason) => {
    ...
    try {
      // 1. Envoyer l'email d'annulation au client AVANT de update
      try {
        const vehicle = vehicles.find(
          (v) => v.id === selectedBooking.details?.vehicleType
        );
        ...
        await fetch("/api/send-cancellation", {
          ...
          body: JSON.stringify({
            booking: {
              ...
              // ✅ CORRIGÉ - Structure trip correcte
              trip: {
                date: selectedBooking.trip?.date,
                time: selectedBooking.trip?.time,
                departure: selectedBooking.trip?.departure,
                arrival: selectedBooking.trip?.arrival,
              },
              ...
              // ✅ CORRIGÉ - Structure details correcte
              details: {
                passengers: selectedBooking.details?.passengers,
                vehicleType: selectedBooking.details?.vehicleType,
                luggage: selectedBooking.details?.luggage,
              },

              vehicle: vehicle || {
                name: selectedBooking.details?.vehicleType,
              },

              pricing: {
                priceEstimate: selectedBooking.pricing?.priceEstimate,
                distanceKm: selectedBooking.pricing?.distanceKm,
              },
            },
            config: widgetConfig,
            reason: reason,
      ...
      // 2. Mettre à jour le statut à "cancelled" dans Firestore
      const result = await updateBookingStatus(selectedBooking.id, "cancelled");

      if (result.success) {
        // 3. Mettre à jour localement le statut
        setBookings((prev) =>
          prev.map((b) =>
        ...
        setResultMessage(
          ...
          } a été annulée. Un email a été envoyé au client.`
        );
    ...
    try {
      ...
      if (result.success) {
          ...
          await fetch("/api/send-confirmation", {
            ...
            body: JSON.stringify({
              booking: {
                ...
                payment: {
                  method: selectedBooking.payment?.method,
                },

                paymentMethod:
                  selectedBooking.payment?.method === "driver"
        ...
        setResultMessage(
          `La réservation de ${
            selectedBooking.customer?.name || "Client"
          } a été confirmée. Un email de confirmation a été envoyé.`
        );
        setShowResultModal(true);
      } else {
        throw new Error(result.error);
  ...
  // Supprimer la réservation de Firestore et localement <--- NOUVEAU
  const handleDeleteBooking = async () => {
    ...
    try {
      ...
      if (result.success) {
        // 2. Mettre à jour localement (retirer la réservation)
        setBookings((prev) =>
      ...
      setShowDeleteModal(false);
      setResultSuccess(false);
      setResultMessage("Une erreur est survenue lors de la suppression de la réservation.");
      setShowResultModal(true);
    }
                ...
                <div className="p-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4 p-4 bg-gradient-to-br from-stone-50 to-stone-100 rounded-xl border border-stone-200">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-medium text-gray-900">
                        {getVehicleName(booking.details?.vehicleType)} -{" "}
                        {booking.details?.passengers || 0} passager(s)
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex justify-between items-center pt-4 border-t border-gray-100">
                    ...
                    <div className="flex gap-2">
                      {booking.status === "pending" && (
                        <>
                          <button
                            onClick={() => handleDeleteClick(booking)} // <--- Bouton de suppression ajouté
                            className="px-2.5 py-2.5 bg-gray-100 text-gray-600 text-sm rounded-lg hover:bg-gray-200 transition-colors font-semibold"
                      ...
                      {booking.status === "cancelled" && (
                        <button
                          onClick={() => handleDeleteClick(booking)} // <--- Bouton de suppression ajouté pour les réservations annulées
                          className="px-5 py-2.5 bg-red-600 text-white text-sm rounded-lg hover:bg-red-700 transition-colors font-semibold"
                        >
                          🗑️ Supprimer
                        </button>
