...
// API pour envoyer le bon de commande par email (client + admin)
// VERSION 4 - PDF COMPACT UNE PAGE
...
function generateOrderPDFBuffer(booking, config) {
  ...
  doc.setFontSize(9);
  doc.setFont('helvetica', 'normal');
  const departText = booking.departure?.address || booking.departureAddress || '-';
  ...
  const totalPrice = pricing.priceEstimate || pricing.total || booking.price || 0;
  doc.setFontSize(24);
  doc.setFont('helvetica', 'bold');
  ...
  if (booking.comments && booking.comments.trim()) {
    ...
    doc.setFont('helvetica', 'normal');
    const commentShort = booking.comments.length > 80 ? booking.comments.substring(0, 80) + '...' : booking.comments;
...
function getClientEmailTemplate(booking, config) {
  const companyName = config?.branding?.companyName || 'VTC Service';
  const primaryColor = config?.branding?.primaryColor || '#2563eb';
  const totalPrice = booking.pricing?.priceEstimate || booking.pricing?.total || booking.price || 0;
  const vehicleName = booking.vehicle?.name || booking.vehicle || '-';
        ...
        <div style="background-color: #f9fafb; border-radius: 8px; padding: 15px; margin: 15px 0;">
          <table width="100%" cellpadding="6" cellspacing="0" style="font-size: 13px;">
            ...
            <tr>
              ...
              <td style="color: #1f2937; text-align: right;">${booking.departure?.address || booking.departureAddress || '-'}</td>
            </tr>
            <tr>
              <td style="color: #6b7280;">Arrivee</td>
              <td style="color: #1f2937; text-align: right;">${booking.arrival?.address || booking.arrivalAddress || '-'}</td>
            </tr>
            <tr>
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 600px; margin: 0 auto; background-color: #ffffff;">
    ...
    <tr>
      <td style="padding: 20px;">
        <div style="background-color: #fef3c7; border-left: 4px solid #f59e0b; padding: 12px; margin-bottom: 15px;">
        ...
        <table width="100%" cellpadding="6" style="font-size: 13px; margin-bottom: 15px;">
          <tr style="background-color: #f9fafb;">
            ...
            <td style="color: #1f2937; font-weight: bold;">${booking.customer?.name || '-'}</td>
          </tr>
          <tr>
          ...
          <tr style="background-color: #f9fafb;">
            ...
            <td style="color: #1f2937; font-weight: bold;">${booking.customer?.phone || '-'}</td>
          </tr>
          <tr>
            <td style="color: #6b7280;">Date</td>
            <td style="color: #1f2937;">${booking.date || '-'} a ${booking.time || '-'}</td>
          </tr>
          <tr style="background-color: #f9fafb;">
            <td style="color: #6b7280;">Depart</td>
            <td style="color: #1f2937;">${booking.departure?.address || booking.departureAddress || '-'}</td>
          </tr>
          <tr>
            <td style="color: #6b7280;">Arrivee</td>
            <td style="color: #1f2937;">${booking.arrival?.address || booking.arrivalAddress || '-'}</td>
          </tr>
          <tr style="background-color: #f9fafb;">
            <td style="color: #6b7280;">Vehicule</td>
        ...
        <div style="background-color: #ecfdf5; border-radius: 8px; padding: 15px; text-align: center;">
          <p style="margin: 0; color: #059669; font-size: 28px; font-weight: bold;">${Number(totalPrice).toFixed(2)} EUR</p>
          <p style="margin: 5px 0 0 0; color: #6b7280; font-size: 11px;">${booking.paymentMethod || ''}</p>
        </div>
        
        ${booking.comments ? `<div style="background-color: #fef3c7; padding: 10px; border-radius: 5px; color: #92400e; margin-top: 15px; font-size: 12px;"><strong>💬</strong> ${booking.comments}</div>` : ''}
      </td>
    </tr>
    <tr>
...
export async function POST(request) {
  console.log('📧 API send-order appelée');
  ...
  try {
    ...
    const pdfBuffer = generateOrderPDFBuffer(booking, config);
    const companyName = config?.branding?.companyName || 'VTC Service';
    ...
    await transporter.sendMail({
      ...
      subject: `Votre demande de reservation - ${companyName}`,
      html: getClientEmailTemplate(booking, config),
      attachments: [{ filename: 'bon-de-commande.pdf', content: pdfBuffer, contentType: 'application/pdf' }],
    });
    
    // Email admin
    const adminAddr = adminEmail || config?.email?.adminEmail;
    if (adminAddr) {
      await transporter.sendMail({
        from: `"${companyName}" <${fromEmail}>`,
        to: adminAddr,
        subject: `Nouvelle reservation - ${booking.customer.name || 'Client'}`,
        html: getAdminEmailTemplate(booking, config),
        attachments: [{ filename: 'bon-de-commande.pdf', content: pdfBuffer, contentType: 'application/pdf' }],
      });
    }
